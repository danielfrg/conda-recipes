...
export ZEPPELIN_HOME="\$(cd "\${CWD}/../share/${PKG_NAME}" && pwd -P)"
echo -e "Setting up ZEPPELIN_HOME to \${ZEPPELIN_HOME} ..."

export ZEPPELIN_CLASSPATH="\$(cd "\${CWD}/../share/${PKG_NAME}/lib" && pwd -P)"/*
echo -e "Setting up ZEPPELIN_CLASSPATH to \${ZEPPELIN_CLASSPATH} ..."

if [[ -z \${@} ]]; then
    echo -e "Launching \${CMD}"
    echo -e ""
    \${ZEPPELIN_HOME}/bin/\${CMD}
else
    echo -e "Launching \${CMD} \"\${@}\""
    echo -e ""
    \${ZEPPELIN_HOME}/bin/\${CMD} "\${@}"
